...
class Glossary(GlossaryType):
	...
	def read(
		...
		**options
		...
		if not format:
			log.error(f"Could not detect input format from file name: \"{filename}\"")
			return False
	...
	def write(
		...
		**options
		...
		try:
			...
			log.critical(f"No write support for \"{format}\" format")
			return
		for key in list(options.keys()):
			if key not in validOptionKeys:
				log.error(
					f"Invalid write option \"{key}\""
					f" given for {format} format"
				)
				del options[key]
	...
	def convert(
		...
		if not outputArgs:
			log.error(f"Writing file \"{outputFilename}\" failed.")
			return
		...
		log.info(f"Writing file \"{finalOutputFile}\" done.")
